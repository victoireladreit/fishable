...
AS $function$
DECLARE
v_old_catch public.catches%ROWTYPE;
    v_new_catch public.catches%ROWTYPE;
    v_user_id uuid;
...
END IF;
    ...
    -- 2. Get the user ID directly from the catch.
    v_user_id := v_old_catch.user_id;

    -- 3. Apply updates using COALESCE to preserve old values if new ones are not provided
    -- Note: p_updates->>'session_id' can be NULL, which is what we want.
    ...
    v_new_catch.session_id := COALESCE((p_updates->>'session_id')::uuid, v_old_catch.session_id);
    ...
    v_new_catch.catch_location_accuracy := COALESCE((p_updates->>'catch_location_accuracy')::real, v_old_catch.catch_location_accuracy);
    v_new_catch.updated_at := NOW();

    -- 4. Get or create the new species ID
SELECT id INTO v_new_species_id FROM public.species_registry WHERE name = v_new_catch.species_name;
IF NOT FOUND THEN
        INSERT INTO public.species_registry (name, scientific_name)
...
UPDATE public.catches SET
                          session_id = v_new_catch.session_id,
                          ...
                          catch_location_accuracy = v_new_catch.catch_location_accuracy,
